...
// 📌 Buscar cliente por RUT y obtener sus mascotas
app.get('/clientes/rut/:rut', async (req, res) => {
  try {
    const rut = req.params.rut;

    const clienteResult = await executeQuery(
      `SELECT id_cliente, nombre, apellido FROM clientes WHERE rut = :rut`,
      { rut }
    );
    ...
    const mascotasResult = await executeQuery(
      `SELECT id_mascota, nombre FROM mascotas WHERE id_cliente = :id_cliente`,
    ...
    res.status(500).json({ error: 'Error al buscar cliente por RUT' });
  }
});


// 📌 Obtener lista de servicios
app.get('/servicios/nombres', async (req, res) => {
  try {
    const result = await executeQuery(`SELECT id_servicio, nombre FROM servicios`);
...
app.get('/profesionales/nombres', async (req, res) => {
  try {
    const result = await executeQuery(`SELECT id_profesional, nombre, apellido FROM profesionales`);
    res.json(result.rows.map(p => ({
      id_profesional: p[0],
      nombre_completo: `${p[1]} ${p[2]}`
    })));
  } catch (err) {
